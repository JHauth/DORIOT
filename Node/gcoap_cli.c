...
// server functions for sending and responding ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
static bool _parse_endpoint(sock_udp_ep_t *remote, char *addr_str, char *port_str)
{
    ipv6_addr_t addr;
...
int gcoap_cli_cmd(int argc, char **argv)
{
    ...
    else if (strcmp(argv[1], "looprequest") == 0)
    {
      char *uri = argv[4];
      int uri_len = strlen(argv[4]);
    ...
    if (((argc == apos + 2) && (code_pos == 0)) ||    /* ping */
        ...
        if (!_send(&buf[0], len, argv[apos], argv[apos+1])) {
            ...
            switch (gcoap_obs_init(&pdu, &buf[0], CONFIG_GCOAP_PDU_BUF_SIZE, &_resources[0])) {
